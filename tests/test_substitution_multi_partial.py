...
class TestMultiplePartialSubstitutions(unittest.TestCase):
    ...
    def test_sequential_partial_substitutions(self):
        ...
        self.crystal.substitute(sel_ca, {'Ca': 'Ba'})
        
        self.assertEqual(self.crystal.species[0], 'Ba')
        self.assertEqual(self.crystal.species[1], 'Ca')  # Not substituted
        
        # Select first C
        sel_c = AtomSelection(self.crystal).by_species('C').by_indices([2])
        self.crystal.substitute(sel_c, {'C': 'Si'})
        
        self.assertEqual(self.crystal.species[2], 'Si')
        self.assertEqual(self.crystal.species[3], 'C')  # Not substituted
    ...
    def test_combined_selection_partial_substitutions(self):
        ...
        self.assertEqual(crystal.species[1], 'Ca')  # Not substituted
        self.assertEqual(crystal.species[2], 'Si')
        self.assertEqual(crystal.species[3], 'C')  # Not substituted
    ...
    def test_partial_substitution_with_position(self):
        ...
        self.assertEqual(crystal.species[0], 'Ba')
        self.assertEqual(crystal.species[1], 'Ca')  # Too far
        
        # Select C atoms near origin
        sel_c = AtomSelection(crystal).by_species('C').near([0, 0, 0], 3.0)
        crystal.substitute(sel_c, {'C': 'Si'})
        
        self.assertEqual(crystal.species[2], 'Si')
        self.assertEqual(crystal.species[3], 'C')  # Too far
