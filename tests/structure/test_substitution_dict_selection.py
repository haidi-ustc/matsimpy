...
class TestSubstitutionWithDict(unittest.TestCase):
    ...
    def test_substitute_with_dict_simple(self):
        ...
        result = crystal.substitute([0, 1], {'Si': 'Ge', 'O': 'S'})
    ...
    def test_substitute_with_dict_multiple(self):
        ...
        result = self.crystal.substitute([0, 1, 2, 3], {'Si': 'Ge', 'O': 'S'})

        self.assertEqual(result.species[0], 'Ge')
        self.assertEqual(result.species[1], 'S')
        self.assertEqual(result.species[2], 'Ge')
        self.assertEqual(result.species[3], 'S')

    def test_substitute_with_dict_partial(self):
        """Test substitution with dict mapping for subset of atoms."""
        result = self.crystal.substitute([0, 2], {'Si': 'Ge'})

        self.assertEqual(result.species[0], 'Ge')
        self.assertEqual(result.species[1], 'O')  # Unchanged
        self.assertEqual(result.species[2], 'Ge')
        self.assertEqual(result.species[3], 'O')  # Unchanged
    ...
    def test_substitute_with_dict_molecule(self):
        ...
        result = self.molecule.substitute([0, 1, 2, 3], {'C': 'N', 'O': 'S', 'N': 'P'})

        self.assertEqual(result.species[0], 'N')
        self.assertEqual(result.species[1], 'S')
        self.assertEqual(result.species[2], 'N')
        self.assertEqual(result.species[3], 'P')
...
class TestSubstitutionWithAtomSelectionAndDict(unittest.TestCase):
    ...
    def test_atom_selection_with_dict(self):
        ...
        result = self.crystal.substitute(sel, {'Si': 'Ge'})
    ...
    def test_atom_selection_with_dict_multiple_species(self):
        ...
        result = self.crystal.substitute(sel, {'Si': 'Ge', 'O': 'S'})
    ...
    def test_atom_selection_chained_with_dict(self):
        ...
        self.assertEqual(result.species[0], 'Ge')
        self.assertEqual(result.species[2], 'Si')  # Not substituted (too far)
    ...
    def test_combined_selections_with_dict(self):
        ...
        combined = sel1 & sel2
        result = self.crystal.substitute(combined, {'Si': 'Ge'})
