...
def _format_crystal_column(
    crystal: Crystal, column: str, index: int, use_mhchem: bool = False
) -> str:
    """
...
def _format_molecule_column(
    molecule: Molecule, column: str, index: int, use_mhchem: bool = False
) -> str:
    """
...
def structures_to_latex_table(
    ...
    if separate_by_type:
        ...
        molecules = [
            s
            for s in structures
            if isinstance(s, Molecule) and not isinstance(s, Crystal)
        ]
...
def save_latex_table(
    structures: List[Union[Crystal, Molecule]], filename: str, **kwargs
) -> None:
    """
