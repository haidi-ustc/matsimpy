...
def read_ASE(filename: str) -> Crystal:
    ...
    if lattice is not None:
        ...
        raise ValueError("ASE file does not contain cell/lattice information. Use read_XYZ for molecules.")
...
def write_ASE(structure, filename: str, title: Optional[str] = None) -> None:
    ...
    if title is None:
        title = structure.formula if hasattr(structure, 'formula') else "MatSimPy Structure"
    ...
    with open(filepath, 'w') as f:
        ...
        if isinstance(structure, Crystal):
            ...
            lattice_str = ' '.join([f"{v:.10f}" for row in lattice.lattice_vectors for v in row])
