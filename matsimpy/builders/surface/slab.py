...
def generate_slab(
    ...
    **kwargs
    ...
    d_spacing = np.linalg.norm(lattice_matrix[0]) / np.sqrt(h**2 + k**2 + l**2) if (h**2 + k**2 + l**2) > 0 else 1.0
    ...
    new_lattice_vectors[2] = new_lattice_vectors[2] * (total_c / np.linalg.norm(new_lattice_vectors[2]))
