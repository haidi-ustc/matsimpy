...
class ParameterSweep:
    ...
    def __init__(
        ...
        if mode not in ['cartesian', 'zip', 'custom']:
            raise ValueError(f"Invalid mode: {mode}. Must be 'cartesian', 'zip', or 'custom'")
    ...
    def _generate_cartesian(self) -> List[Dict[str, Any]]:
        ...
        for name in transform_names:
            ...
            for param_combo in itertools.product(*param_value_lists):
                combo_dict = {param_names[i]: param_combo[i] 
        ...
        for idx in range(max_length):
            ...
            for name in transform_names:
                ...
                for param_name, param_values in param_dict.items():
                    param_combo[param_name] = param_values[min(idx, len(param_values) - 1)]
