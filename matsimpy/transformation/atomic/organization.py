...
def sort_atoms(
    ...
    # Sort
    sorted_indices = [idx for idx, _ in sorted(keys, key=lambda x: x[1], reverse=reverse)]
...
def perturb_positions(
    ...
    if isinstance(structure, Crystal):
        ...
        frac_perturbations = np.dot(perturbations, np.linalg.inv(structure.lattice.lattice_vectors))
