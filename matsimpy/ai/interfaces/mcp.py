...
class MCPInterface(AIInterface):
    ...
    def _connect_websocket(self, config: Dict[str, Any]) -> None:
        ...
        if not url:
            raise ValueError(
                "WebSocket transport requires 'url' in config or server_url"
            )
    ...
    def _call_http(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if session:
            response = session.post(f"{url}/jsonrpc", json=request, timeout=30)
        else:
            response = requests.post(f"{url}/jsonrpc", json=request, timeout=30)
