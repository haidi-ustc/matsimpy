...
class PropertyPrediction(AIOperation):
    ...
    def predict_band_gap(self, crystal: Crystal) -> float:
        ...
        result = self.interface.call(
            operation="predict_band_gap", inputs={"structure": structure_data}
        )
    ...
    def predict_formation_energy(self, crystal: Crystal) -> float:
        ...
        result = self.interface.call(
            operation="predict_formation_energy", inputs={"structure": structure_data}
        )
    ...
    def predict_bulk_modulus(self, crystal: Crystal) -> float:
        ...
        result = self.interface.call(
            operation="predict_bulk_modulus", inputs={"structure": structure_data}
        )

        return float(result.get("bulk_modulus", 0.0))

    def predict_properties(
        self, crystal: Crystal, properties: Optional[List[str]] = None
    ) -> Dict[str, float]:
        """
        ...
        result = self.interface.call(operation="predict_properties", inputs=inputs)
