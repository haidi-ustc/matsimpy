...
class SkillManager:
    ...
    def __init__(self, skills: dict[str, Skill] | None = None):
        ...
        self._keyword_map: dict[str, str] = self._build_keyword_map()

    def _build_keyword_map(self) -> dict[str, str]:
        """Build keyword→skill_name mapping from registered skill keywords."""
        mapping: dict[str, str] = {}
        for name, skill in self._skills.items():
            for kw in skill.keywords:
                if kw in mapping and mapping[kw] != name:
                    import warnings
                    warnings.warn(
                        f"Keyword '{kw}' claimed by both '{mapping[kw]}' and '{name}' — "
                        f"'{name}' wins (registered later)"
                    )
                mapping[kw] = name
        return mapping
