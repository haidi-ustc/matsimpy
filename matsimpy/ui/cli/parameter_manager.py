        ...
        if text:
            try:
                if self.param_def.type == bool:
                    if text.lower() not in ['true', 'false', 'yes', 'no', 'y', 'n', '1', '0']:
                        raise ValidationError(message='Please enter true/false, yes/no, y/n, or 1/0')
                elif self.param_def.type == dict:
                    json.loads(text)
                else:
                    self.param_def.type(text)
            except (ValueError, json.JSONDecodeError):
                raise ValidationError(message=f'Invalid {self.param_def.type.__name__} format')
        ...
        if text and self.param_def.validator:
            try:
                ...
                if not self.param_def.validator(value):
                    raise ValidationError(message='Value does not meet validation criteria')
            except:
                pass
...
class StructureManager:
    ...
    def get_structure_file(self, prompt_text: str = "Enter structure file path") -> Optional[str]:
        ...
        while True:
            ...
            if os.path.exists(filepath) and os.path.isfile(filepath):
                ...
                print_formatted_text(FormattedText([
                    ('class:error', f'File not found: {filepath}')
                ]), style=self.style)
                
                # Offer to retry or cancel
                retry = self.session.prompt(
                    FormattedText([
                        ('class:prompt', 'Try again? (y/n): ')
                    ]),
                    style=self.style
                ).strip().lower()
    ...
    def load_structure(self, filepath: str) -> Optional[Union[Crystal, Molecule]]:
        ...
        try:
            ...
            print_formatted_text(FormattedText([
                ('class:success', f'Successfully loaded structure from: {filepath}')
            ]), style=self.style)
            return structure
        except Exception as e:
            print_formatted_text(FormattedText([
                ('class:error', f'Error loading structure: {str(e)}')
            ]), style=self.style)
            return None
...
class CLIParameterManager:
    ...
    def _show_parameter_summary(self):
        ...
        for param in self.parameter_definitions:
            ...
            if param.choices:
                print_formatted_text(FormattedText([
                    ('class:info', f'  Choices: {", ".join(str(c) for c in param.choices)}')
                ]), style=self.style)
    ...
    def _get_parameters_json(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            print_formatted_text(FormattedText([
                ('class:info', 'Detected multi-line JSON. Continue entering lines.'),
                ('', '\n'),
                ('class:info', 'Type "END" on a new line or press Enter twice to finish:')
            ]), style=self.style)
    ...
    def _load_parameters_from_file(self) -> Dict[str, Any]:
        ...
        filepath = self.session.prompt(
            FormattedText([
                ('class:prompt', 'Enter parameter file path: ')
            ]),
            style=self.style,
            completer=path_completer
        ).strip()
        ...
        try:
            ...
            validated_params = self._validate_parameters(input_params)
            print_formatted_text(FormattedText([
                ('class:success', f'Successfully loaded parameters from: {filepath}')
            ]), style=self.style)
            return validated_params
    ...
    def _use_previous_values(self) -> Dict[str, Any]:
        ...
        for key, value in params.items():
            print_formatted_text(FormattedText([
                ('class:menu_code', f'  {key}:'),
                ('class:menu_item', f' {value}')
            ]), style=self.style)
            ...
            while True:
                ...
                if param.name in self._param_history and self._param_history[param.name]:
                    print_formatted_text(FormattedText([
                        ...
                        ('class:menu_item', ', '.join(str(v) for v in self._param_history[param.name][:3]))
                    ]), style=self.style)
                ...
                value = self.session.prompt(
                    ...
                    style=self.style,
                    completer=completer,
                    validator=ParameterValidator(param) if param.required or param.validator else None
                ).strip()
                ...
                try:
                    if param.type == bool:
                        ...
                        if value.lower() in ['true', 'yes', 'y', '1']:
                            ...
                            raise ValueError("Please enter true/false, yes/no, y/n, or 1/0")
                    elif param.type == dict:
                    ...
                    if param.validator and not param.validator(converted_value):
                        print_formatted_text(FormattedText([
                            ('class:error', 'Value does not meet validation criteria')
                        ]), style=self.style)
                        continue
                    ...
                    print_formatted_text(FormattedText([
                        ('class:error', f'Error: {str(e)}')
                    ]), style=self.style)
        
        return params

    # Maintain original interface methods
    def define_structure_parameter(self, name: str = "structure_file", description: str = "Input structure file path", required: bool = True) -> None:
    ...
    def get_structure_from_params(self, params: Dict[str, Any], param_name: str = "structure_file") -> Optional[Union[Crystal, Molecule]]:
    ...
    def get_input_structure(self) -> Optional[Union[Crystal, Molecule]]:
        ...
        filepath = self.structure_manager.get_structure_file("Enter input structure file path")
        if filepath:
            return self.structure_manager.load_structure(filepath)
        return None

    def define_parameter(self, name: str, description: str, param_type: type, required: bool = False, 
                        default: Any = None, validator: Optional[Callable[[Any], bool]] = None,
                        choices: Optional[List[Any]] = None, completer: Optional[Any] = None) -> None:
        """Define a parameter for the CLI interface with enhanced features."""
        self.parameter_definitions.append(ParameterDefinition(
            name=name,
            type=param_type,
            description=description,
            default=default,
    ...
    def save_parameters_to_file(self, params: Dict[str, Any], filepath: Optional[str] = None) -> str:
        ...
        try:
            ...
            print_formatted_text(FormattedText([
                ('class:success', f'Parameters saved to: {filepath}')
            ]), style=self.style)
            return filepath
            
        except Exception as e:
            print_formatted_text(FormattedText([
                ('class:error', f'Error saving parameters: {str(e)}')
            ]), style=self.style)
            raise
