...
class AdvancedInteractiveMenu:
    ...
    def get_command_completer(self) -> WordCompleter:
        ...
        current_codes = [item['code'][1:-1] for item in current_items]  # Remove brackets
    ...
    def format_grouped_main_menu(self) -> FormattedText:
        ...
        for group_name, codes in groups.items():
            ...
            group_items = [items_by_code[code] for code in codes if code in items_by_code]
    ...
    def format_menu_items(self, items: List[Dict]) -> FormattedText:
        ...
        lines.append(('class:header', f"Available {level_names[self.current_level]} (Level {self.current_level}):"))
        lines.append(('', '\n'))
        ...
        if self.current_level == 3:
            lines.append(('class:info', "Status: ✓=Implemented ⚠=Partial ✗=Not Implemented ?=Error"))
            lines.append(('', '\n'))
            lines.append(('class:separator', "-" * 50))
            lines.append(('', '\n'))
    ...
    def get_current_menu_items(self) -> List[Dict]:
        ...
        if self.current_level == 1:
            ...
            if self.current_parent:
                return self.menu_data['menu_structure']['level2_items'].get(self.current_parent, [])
        elif self.current_level == 3:
            if self.current_parent:
                return self.menu_data['menu_structure']['level3_items'].get(self.current_parent, [])
        return []
    ...
    def display_search_results(self, query: str, results: List[Dict]):
        ...
        if not results:
            print_formatted_text(FormattedText([
                ('class:error', f'No results found for: {query}')
            ]), style=self.style)
            return
        
        lines = []
        lines.append(('class:header', f'Search Results for "{query}" ({len(results)} found)'))
        lines.append(('', '\n'))
        lines.append(('class:separator', '-' * 50))
        lines.append(('', '\n'))
            ...
            if items:
                ...
                for item in items:
                    ...
                    if item.get('parent'):
                        ...
                        if parent_item:
                            lines.append(('class:info', f" (under {parent_item['name']})"))
                    lines.append(('', '\n'))
                lines.append(('', '\n'))
    ...
    def execute_system_command(self, command: str) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                print_formatted_text(FormattedText([
                    ('class:warning', f"Command exited with code: {result.returncode}")
                ]), style=self.style)
            ...
            print_formatted_text(FormattedText([
                ('class:error', f"Error executing command: {e}")
            ]), style=self.style)
            return False
    ...
    def show_operation_details(self, item: Dict):
        ...
        if item.get('interface'):
            ...
            lines.append(('class:info', f'Status: {status_descriptions.get(status, "Unknown status")}'))
            lines.append(('', '\n'))
            ...
            if status == "implemented":
                ...
                lines.append(('class:warning', 'This operation is not yet fully implemented.'))
                lines.append(('', '\n'))
                lines.append(('class:info', 'Press [b] to go back or try to execute anyway.'))
                lines.append(('', '\n'))
                print_formatted_text(FormattedText(lines), style=self.style)
    ...
    def execute_interface(self, item: Dict) -> bool:
        ...
        if not interface_path:
            print_formatted_text(FormattedText([
                ...
                ('', '\n'),
                ('class:info', '1. Create the interface module in matsimpy/ui/cli/interfaces/'),
                ('class:info', '2. Define the required function with proper signature'),
                ('class:info', '3. Update the menu configuration with the interface path'),
                ('', '\n'),
        ...
        try:
            ...
            if hasattr(module, function_name):
                ...
                try:
                   result = interface_func(style=self.style)
                    ...
                    print_formatted_text(FormattedText([
                        ('', '\n'),
                        ('class:warning', 'Execution interrupted by user. Returning to main menu...'),
                        ('', '\n')
                    ]), style=self.style)
                    return False
                ...
                if isinstance(result, dict):
                    if result.get('status'):
                        print_formatted_text(FormattedText([
                            ('class:success', result.get('message', 'Operation completed successfully')),
                            ('', '\n')
                        ]), style=self.style)
                    else:
                        print_formatted_text(FormattedText([
                            ('class:error', result.get('message', 'Operation failed')),
                            ('', '\n')
                        ]), style=self.style)
                else:
                    print_formatted_text(FormattedText([
                ...
                print_formatted_text(FormattedText([
                    ('class:error', f'Function "{function_name}" not found in module "{module_name}"'),
                    ('', '\n'),
                    ('class:warning', 'Module exists but function is not implemented.'),
                    ('', '\n'),
                    ('class:info', 'To implement this function:'),
                    ('', '\n'),
                    ('class:info', f'1. Open file: {module_name.replace(".", "/")}.py'),
                    ('class:info', f'2. Add function: def {function_name}(style=None):'),
                    ...
                    ('class:info', '    return {"status": True, "message": "Success"}'),
                    ('', '\n'),
                    ('class:warning', 'Press Enter to continue...'),
                    ('', '\n')
                ]), style=self.style)
            ...
            if "No module named" in error_msg:
                ...
                print_formatted_text(FormattedText([
                    ('class:error', f'Module not implemented: {missing_module}'),
                    ('', '\n'),
                    ('class:warning', 'This feature is not yet implemented.'),
                    ('', '\n'),
                    ('class:info', 'To implement this module:'),
                    ('', '\n'),
                    ('class:info', f'1. Create directory: matsimpy/ui/cli/interfaces/{module_path.split("/")[-1]}/'),
                    ('class:info', f'2. Create file: {module_path.split("/")[-1]}.py'),
                    ...
                    ('class:info', f'matsimpy/ui/cli/interfaces/{module_path.split("/")[-1]}/'),
                ...
                print_formatted_text(FormattedText([
                    ...
                    ('', '\n'),
                    ('class:warning', 'Make sure the interface module is installed and accessible'),
                    ('', '\n'),
            ...
            print_formatted_text(FormattedText([
                ...
                ('', '\n'),
                ('class:warning', 'Unexpected error occurred during execution.'),
                ('', '\n'),
                ('class:info', 'Troubleshooting steps:'),
                ('class:info', '1. Check the interface implementation for errors'),
                ('class:info', '2. Verify all required dependencies are available'),
                ('class:info', '3. Check the function signature and return values'),
                ('class:info', '4. Review the error message for specific issues'),
                ('', '\n'),
                ('class:warning', 'Press Enter to continue...'),
                ('', '\n')
            ]), style=self.style)
    ...
    def show_history(self):
        ...
        if not self.session.navigation_history:
            print_formatted_text(FormattedText([
                ('class:info', 'No navigation history yet.')
            ]), style=self.style)
            return
    ...
    def show_help(self):
        ...
        help_text = FormattedText([
            ...
            ('', '\n'),
            ('class:menu_code', '  [number]'), ('class:menu_item', '   - Enter menu item (e.g., 11, s11, s111)'),
            ('', '\n'),
            ('class:menu_code', '  b, back'), ('class:menu_item', '  - Go back to previous level'),
            ('', '\n'),
            ('class:menu_code', '  q, quit'), ('class:menu_item', '  - Exit program'),
            ('', '\n'),
            ('', '\n'),
            ('class:menu_item', 'Advanced Features:'),
            ('', '\n'),
            ('class:menu_code', '  search <term>'), ('class:menu_item', '     - Search menu items'),
            ('', '\n'),
            ('class:menu_code', '  s <term>'), ('class:menu_item', '         - Quick search'),
            ('', '\n'),
            ('class:menu_code', '  history'), ('class:menu_item', '         - Show navigation history'),
            ('', '\n'),
            ('class:menu_code', '  stats'), ('class:menu_item', '           - Show menu statistics'),
            ('', '\n'),
            ('class:menu_code', '  impl'), ('class:menu_item', '            - Show detailed implementation status'),
            ('', '\n'),
            ('class:menu_code', '  clear'), ('class:menu_item', '           - Clear screen'),
            ('', '\n'),
            ('class:menu_code', '  goto <code>'), ('class:menu_item', '     - Jump directly to any item'),
            ('', '\n'),
            ('class:menu_code', '  ! <command>'), ('class:menu_item', '    - Execute system command'),
            ('', '\n'),
            ('', '\n'),
            ('class:menu_item', 'Interface Status Indicators:'),
            ('', '\n'),
            ('class:success', '  ✓'), ('class:menu_item', ' - Fully implemented and ready to use'),
            ('', '\n'),
            ('class:warning', '  ⚠'), ('class:menu_item', ' - Module exists but function needs implementation'),
            ('', '\n'),
            ('class:error', '  ✗'), ('class:menu_item', ' - Module not implemented yet'),
            ('', '\n'),
            ('class:error', '  ?'), ('class:menu_item', ' - Error checking implementation status'),
            ('', '\n'),
            ('', '\n'),
            ...
            ('', '\n'),
            ('class:info', '  • When a module is not implemented, you\'ll see detailed'),
            ('', '\n'),
            ('class:info', '    instructions on how to implement it'),
            ('', '\n'),
            ('class:info', '  • Use "stats" command to see overall implementation status'),
            ('', '\n'),
            ('class:info', '  • Check operation details for specific implementation info'),
            ('', '\n'),
            ('', '\n'),
            ('class:menu_item', 'System Commands:'),
            ('', '\n'),
            ('class:menu_code', '  ! ls'), ('class:menu_item', '           - List files'),
            ('', '\n'),
            ('class:menu_code', '  ! pwd'), ('class:menu_item', '          - Show current directory'),
            ('', '\n'),
            ('class:menu_code', '  ! ps'), ('class:menu_item', '           - Show processes'),
            ('', '\n'),
            ('class:menu_code', '  ! <any_cmd>'), ('class:menu_item', '    - Execute any system command'),
            ('', '\n'),
            ('class:separator', '═' * 50),
            ('', '\n')
        ])
    ...
    def show_statistics(self):
        ...
        lines.append(('', '\n'))
        lines.append(('class:info', f'  Start Time: {self.session.start_time.strftime("%Y-%m-%d %H:%M:%S")}'))
        lines.append(('', '\n'))
        lines.append(('class:info', f'  Duration: {session_time.seconds // 60}m {session_time.seconds % 60}s'))
        lines.append(('', '\n'))
        lines.append(('class:info', f'  Commands Executed: {self.session.commands_executed}'))
        lines.append(('', '\n'))
        lines.append(('class:info', f'  Navigation History: {len(self.session.navigation_history)} items'))
        lines.append(('', '\n'))
        ...
        level2_count = sum(len(items) for items in self.menu_data['menu_structure']['level2_items'].values())
        level3_count = sum(len(items) for items in self.menu_data['menu_structure']['level3_items'].values())
        ...
        status_counts = {"implemented": 0, "partial": 0, "not_implemented": 0, "error": 0}
        total_interfaces = 0
        ...
        if total_interfaces > 0:
            ...
            lines.append(('', '\n'))
            lines.append(('class:success', f'  ✓ Implemented: {status_counts["implemented"]} ({status_counts["implemented"]/total_interfaces*100:.1f}%)'))
            lines.append(('', '\n'))
            lines.append(('class:warning', f'  ⚠ Partial: {status_counts["partial"]} ({status_counts["partial"]/total_interfaces*100:.1f}%)'))
            lines.append(('', '\n'))
            lines.append(('class:error', f'  ✗ Not Implemented: {status_counts["not_implemented"]} ({status_counts["not_implemented"]/total_interfaces*100:.1f}%)'))
            lines.append(('', '\n'))
            if status_counts["error"] > 0:
                lines.append(('class:error', f'  ? Error: {status_counts["error"]} ({status_counts["error"]/total_interfaces*100:.1f}%)'))
                lines.append(('', '\n'))
        else:
            lines.append(('class:warning', '  No interfaces found in menu configuration'))
            lines.append(('', '\n'))
        ...
        if self.session.last_accessed:
            sorted_access = sorted(self.session.last_accessed.items(), 
                                 key=lambda x: x[1], reverse=True)[:5]
            for code, access_time in sorted_access:
                item = self.find_menu_item(code)
                if item:
                    lines.append(('class:info', f'  {item["name"]} ({code}) - {access_time.strftime("%H:%M:%S")}'))
                    lines.append(('', '\n'))
        else:
        ...
        if command in ['q', 'quit', 'exit']:
            ...
            if args:
                ...
                if not self.navigate_to_item(code):
                    print_formatted_text(FormattedText([
                        ('class:error', f'Cannot navigate to: {code}')
                    ]), style=self.style)
                    input("Press Enter to continue...")
                else:
            ...
            if not self.navigate_to_item(command):
                print_formatted_text(FormattedText([
                    ...
                    ('', '\n'),
                    ('class:info', 'Type "h" for help or use Tab for completion')
                ]), style=self.style)
                input("Press Enter to continue...")
            else:
    ...
    def run(self):
        ...
        summary = FormattedText([
            ...
            ('', '\n'),
            ('class:info', f'Duration: {session_time.seconds // 60}m {session_time.seconds % 60}s'),
            ('', '\n'),
    ...
    def show_implementation_status(self):
        ...
        lines.append(('', '\n'))
        lines.append(('class:success', f'✓ Implemented: {len(status_groups["implemented"])}'))
        lines.append(('', '\n'))
        lines.append(('class:warning', f'⚠ Partial: {len(status_groups["partial"])}'))
        lines.append(('', '\n'))
        lines.append(('class:error', f'✗ Not Implemented: {len(status_groups["not_implemented"])}'))
        lines.append(('', '\n'))
        ...
        for status, items in status_groups.items():
            if items:
                ...
                lines.append((f'class:{status_colors[status]}', f'{status_names[status]} ({len(items)} items):'))
                lines.append(('', '\n'))
