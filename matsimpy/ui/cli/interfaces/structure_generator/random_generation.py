...
def crystal_structure(style: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            composition_dict = {el: int(count) for el, count in comp.composition.items()}
        ...
        return {"status": True, "message": f"Random crystal created successfully with {len(structure)} atoms.", "data": structure}
    ...
    try:
        ...
        for i, species in enumerate(species_list):
            ...
            while attempts < max_attempts:
                ...
                pos = [random.uniform(0, 10), random.uniform(0, 10), random.uniform(0, 10)]
            ...
            if attempts >= max_attempts:
                return {"status": False, "message": f"Failed to place atom {i+1} after {max_attempts} attempts. Try increasing max_attempts or decreasing min_distance."}
        ...
        return {"status": True, "message": f"Molecular structure created successfully with {len(structure)} atoms.", "data": structure}
        
    except Exception as e:
        return {"status": False, "message": f"Error in molecular structure interface: {e}"} 
