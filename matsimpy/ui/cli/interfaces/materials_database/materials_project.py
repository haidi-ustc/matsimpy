...
def structure_search(style: Optional[str] = None) -> None:
    ...
    try:
        ...
        search_params = {"fields": ["structure", "material_id", "formula_pretty", "band_gap"]}
        
        if query.startswith(('mp-', 'mvc-')):
            # Material ID search
            search_params["material_ids"] = [query]
            print(f"Searching by material ID: {query}")
        elif any(char.isdigit() for char in query) and not any(char.isalpha() for char in query.replace('-', '')):
    ...
    try:
        ...
        with MPRester(api_key) as mpr:
            try:
                ...
                print(f"{'ID':<12} {'Formula':<15} {'Band Gap':<10} {'Form. E':<10} {'E Hull':<8}")
                print("-" * 65)
                
                for doc in docs:
                    material_id = doc.material_id
                    formula = getattr(doc, 'formula_pretty', 'N/A')
                    band_gap = f"{doc.band_gap:.3f}" if hasattr(doc, 'band_gap') and doc.band_gap is not None else "N/A"
                    form_energy = f"{doc.formation_energy_per_atom:.3f}" if hasattr(doc, 'formation_energy_per_atom') and doc.formation_energy_per_atom is not None else "N/A"
                    e_hull = f"{doc.energy_above_hull:.3f}" if hasattr(doc, 'energy_above_hull') and doc.energy_above_hull is not None else "N/A"
                    
                    print(f"{material_id:<12} {formula:<15} {band_gap:<10} {form_energy:<10} {e_hull:<8}")
                ...
                for doc in docs:
                    ...
                    for field in ["formula_pretty", "band_gap", "formation_energy_per_atom", "energy_above_hull"]:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("Note: This is for Materials Project API integration, not matsimpy core.")
                return
        except ImportError:
        ...
        if len(elements) > 5:
            print("Warning: Phase diagrams with >5 elements are complex and may not display well.")
        ...
        with MPRester(api_key) as mpr:
            try:
                ...
                for i, entry in enumerate(stable_entries, 1):
                    ...
                    print(f"{i:2d}. {material_id:<12} {formula:<15} E={formation_energy:.3f} eV/atom")
                
                # Find metastable phases
                unstable_entries = [e for e in pd.all_entries if e not in stable_entries]
                metastable_entries = [e for e in unstable_entries if pd.get_e_above_hull(e) <= 0.1]
                
                if metastable_entries:
                    print(f"\n=== Metastable Phases (E_hull ≤ 0.1 eV/atom) ===")
                    for i, entry in enumerate(metastable_entries[:10], 1):  # Show first 10
                        material_id = getattr(entry, 'entry_id', 'N/A')
                        formula = entry.composition.reduced_formula
                        e_hull = pd.get_e_above_hull(entry)
                        
                        print(f"{i:2d}. {material_id:<12} {formula:<15} E_hull={e_hull:.3f} eV/atom")
                    
                    if len(metastable_entries) > 10:
                        print(f"    ... and {len(metastable_entries) - 10} more metastable phases")
                
                # Generate phase diagram plot
                if save_plot and len(elements) <= 4:  # Only plot for reasonable number of elements
                    try:
                        ...
                        plotter.get_plot().savefig(plot_file, dpi=300, bbox_inches='tight')
