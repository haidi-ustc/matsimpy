...
def simple_supercell(style: Optional[str] = None) -> None:
    ...
    try:
        ...
        input_structure = param_manager.get_structure_from_params(params, "structure_file")
        ...
        if not isinstance(input_structure, Crystal):
            print("Error: Supercell creation only works for Crystal structures, not Molecules.")
            return
        ...
        try:
            ...
            if len(size_parts) == 1:
                ...
                scaling_matrix = np.array([[size, 0, 0],
                                         [0, size, 0],
                                         [0, 0, size]])
            else:
                scaling_matrix = np.array([[int(size_parts[0]), 0, 0],
            ...
            print(f"Lattice parameters: a={supercell.lattice.a:.3f}, b={supercell.lattice.b:.3f}, c={supercell.lattice.c:.3f}")
            print(f"Lattice angles: α={supercell.lattice.alpha:.3f}°, β={supercell.lattice.beta:.3f}°, γ={supercell.lattice.gamma:.3f}°")
