...
def add_atoms(style: Optional[str] = None) -> None:
    ...
    try:
        ...
        if isinstance(input_structure, Crystal):
            print(f"Lattice parameters: a={input_structure.lattice.a:.3f}, b={input_structure.lattice.b:.3f}, c={input_structure.lattice.c:.3f}")
            print(f"Lattice angles: α={input_structure.lattice.alpha:.3f}°, β={input_structure.lattice.beta:.3f}°, γ={input_structure.lattice.gamma:.3f}°")
        ...
        if position_type == "coordinates":
            ...
            for i, (element, coords) in enumerate(zip(elements, coordinates)):
                if isinstance(working_structure, Crystal):
                    ...
                    frac_coords = np.dot(coords, working_structure.lattice.inv_matrix).tolist()
            ...
            for i, (element, coords) in enumerate(zip(elements, coordinates)):
                if isinstance(working_structure, Crystal):
                    ...
                    if any(c > 1.0 for c in coords):
                        frac_coords = np.dot(coords, working_structure.lattice.inv_matrix).tolist()
                        print(f"Converted {element} coordinates to fractional: {frac_coords}")
                    else:
                        frac_coords = coords
                    working_structure.add_atom(element, frac_coords)
                else:
            ...
            for i, element in enumerate(elements):
                if isinstance(working_structure, Crystal):
                    ...
                    print(f"Added {element} atom {i+1} at random position: {cart_coords}")
                else:
        ...
        if isinstance(final_structure, Crystal):
            print(f"Lattice parameters: a={final_structure.lattice.a:.3f}, b={final_structure.lattice.b:.3f}, c={final_structure.lattice.c:.3f}")
            print(f"Lattice angles: α={final_structure.lattice.alpha:.3f}°, β={final_structure.lattice.beta:.3f}°, γ={final_structure.lattice.gamma:.3f}°")
        ...
        print(f"Successfully added {len(elements)} atoms. Total atoms: {len(final_structure)}")
...
def move_atoms_interface(style: Optional[str] = None) -> None:
    ...
    try:
        ...
        print(f"Selected {len(selected_indices)} atoms: {[i+1 for i in selected_indices]}")
        ...
        for i, idx in enumerate(selected_indices):
            if isinstance(input_structure, Crystal):
                ...
                species = input_structure.species[idx]
            print(f"  {i+1:2d}. Atom {idx+1:2d}: {species:>4} at [{pos[0]:6.3f}, {pos[1]:6.3f}, {pos[2]:6.3f}]")
        ...
        if isinstance(moved_structure, Crystal):
            print(f"Lattice parameters: a={moved_structure.lattice.a:.3f}, b={moved_structure.lattice.b:.3f}, c={moved_structure.lattice.c:.3f}")
...
def delete_atoms(style: Optional[str] = None) -> None:
    ...
    try:
        ...
        print(f"Selected {len(selected_indices)} atoms: {[i+1 for i in reversed(selected_indices)]}")
        ...
        for idx in selected_indices:
            try:
                ...
                print(f"Warning: Atom {idx+1} no longer exists (may have been deleted already)")
