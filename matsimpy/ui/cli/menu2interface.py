...
class EnhancedMenuParser:
    ...
    def format_interface_structure(self) -> str:
        ...
        for l1_item, l2_modules in sorted(structure.items()):
            ...
            for i, (l2_module, functions) in enumerate(module_items):
                ...
                for j, func_info in enumerate(functions[:3]):  # Show first 3 functions
                    ...
                    function_name = (
                        func_info["interface"].split(":")[1]
                        if ":" in func_info["interface"]
                        else "main"
                    )
                    lines.append(
                        f"│   {func_line_prefix}       def {function_name}():  # {func_info['name']} ({func_info['code']})"
                    )
                ...
                if len(functions) > 3:
                    if is_last_module:
                        ...
                        func_line_prefix = "│   "
                    lines.append(
                        f"│   {func_line_prefix}       ... and {len(functions) - 3} more functions"
                    )
        ...
        total_functions = sum(
            len(functions)
            for l2_modules in structure.values()
    ...
    def format_complete_menu(self) -> str:
        ...
        lines.append("=" * 80)
        lines.append(
            "MatSimPy v2.0 - Complete Menu Structure with Auto-Generated Interfaces"
        )
        lines.append(
            "Interface Format: matsimpy.ui.cli.interfaces.l1_item.l2_item:l3_function"
        )
        lines.append("=" * 80)
        ...
        interfaces_count = sum(
            1 for item in self.menu_items.values() if item.level == 3 and item.interface
        )

        lines.append(
            f"Total Structure: {len(self.level1_items)} Level 1, {level2_count} Level 2, {level3_count} Level 3"
        )
        lines.append(
            f"Auto-Generated Interfaces: {interfaces_count} operations with interfaces"
        )
        ...
        for level1_item in self.level1_items:
            ...
            for level2_item in level2_items:
                ...
                if level3_items:
                    for level3_item in level3_items:
                        interface_info = (
                            f" -> {level3_item.interface}"
                            if level3_item.interface
                            else " -> No Interface"
                        )
                        lines.append(
                            f"    [LEVEL 3] {level3_item.code} {level3_item.name}{interface_info}"
                        )
                else:
    ...
    def format_statistics(self) -> str:
        ...
        for cat_name, (start, end) in categories.items():
            ...
            coverage = (
                f"{cat_interfaces}/{cat_operations}" if cat_operations > 0 else "0/0"
            )
            percentage = (
                f"({cat_interfaces/cat_operations*100:.1f}%)"
                if cat_operations > 0
                else "(0%)"
            )
        ...
        try:
            ...
            data = {
                "metadata": {
                    ...
                    "level2_count": sum(
                        len(items) for items in self.level2_items.values()
                    ),
                    "level3_count": sum(
                        len(items) for items in self.level3_items.values()
                    ),
                    "interfaces_count": sum(
                        1
                        for item in self.menu_items.values()
    ...
    def search_menu(self, query: str) -> List[MenuItem]:
        ...
        for item in self.menu_items.values():
            if (
                ...
                or (item.interface and query_lower in item.interface.lower())
            ):
                results.append(item)
        ...
        for level in [1, 2, 3]:
            ...
            if items:
                ...
                for item in items:
                    ...
                    lines.append(
                        f"  {item.code} {item.name}{parent_info}{interface_info}"
                    )
