...
class SymmetryAnalyzer:
    ...
    def analyze_molecule(self, molecule: Molecule, tolerance: float = 0.1) -> Dict[str, Any]:
        ...
        # Detect point group
        point_group = self._detect_point_group(centered_positions, molecule.species, tolerance)
        ...
        return {
            ...
            'inversion_center': self._has_inversion_center(centered_positions, tolerance),
    ...
    def _detect_point_group(self, positions: np.ndarray, species: List[str], 
                           tolerance: float) -> str:
        """
        ...
        return self._point_group_from_elements(rotation_axes, mirror_planes, has_inversion)
    ...
    def _is_linear(self, positions: np.ndarray, tolerance: float) -> bool:
        ...
        for pos in centered:
            dist_to_axis = np.linalg.norm(pos - np.dot(pos, principal_axis) * principal_axis)
    ...
    def _has_tetrahedral_symmetry(self, positions: np.ndarray, tolerance: float) -> bool:
    ...
    def _find_rotation_axes(self, positions: np.ndarray, tolerance: float) -> List[Dict]:
    ...
    def _find_rotation_axis(self, positions: np.ndarray, center: np.ndarray,
                           order: int, tolerance: float) -> Optional[np.ndarray]:
    ...
    def _has_rotation_symmetry(self, positions: np.ndarray, center: np.ndarray,
                              axis: np.ndarray, order: int, tolerance: float) -> bool:
    ...
    def _find_mirror_planes(self, positions: np.ndarray, tolerance: float) -> List[Dict]:
    ...
    def _has_mirror_symmetry(self, positions: np.ndarray, center: np.ndarray,
                           normal: np.ndarray, tolerance: float) -> bool:
    ...
    def _point_group_from_elements(self, rotation_axes: List[Dict], 
                                  mirror_planes: List[Dict], 
                                  has_inversion: bool) -> str:
        """Determine point group from symmetry elements."""
    ...
    def _get_point_group_order(self, point_group: str) -> int:
        ...
        order_map = {
            ...
            'D2d': 8, 'D3d': 12, 'D4d': 16, 'D5d': 20, 'D6d': 24,
            'S2': 2, 'S4': 4, 'S6': 6, 'S8': 8,
            'T': 12, 'Th': 24, 'Td': 24,
            'O': 24, 'Oh': 48,
            'I': 60, 'Ih': 120,
...
def analyze_symmetry(structure: Union[Crystal, Molecule], 
                    symprec: float = 1e-5,
